...
Future<void> main() async {
  final validAcblFiles = ['./assets/acbl/ACBL.acbl'];

  for (final filePath in validAcblFiles) {
    test('isValidFormat returns true for valid ACBL file: $filePath', () {
      final acblFile = File(filePath);
      final bytes = acblFile.readAsBytesSync();
      expect(AdobeColorBookLegacy.isValidFormat(bytes), isTrue);
    });

    test('read acbl file: $filePath', () {
      final acblFile = File(filePath);
